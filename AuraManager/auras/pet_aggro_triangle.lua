...
ns.auras["pet_aggro_triangle"] = {
    ...
    if not aura_env.last or GetTime() - aura_env.last > 0.2 then
        ...
        -- Function to check if pet is tanking a unit
        local function checkUnit(unit)
            if UnitExists(unit) and not UnitIsDeadOrGhost(unit) and GetRaidTargetIndex(unit) == 4 then
                -- Check if pet exists and is tanking this unit
                if UnitExists("pet") and UnitDetailedThreatSituation("pet", unit) then
                    -- Found a triangle-marked unit that pet is tanking
        ...
        -- Pet is not tanking any triangle-marked unit
